...
export default function Landing() {
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--bg-deep)', fontFamily: 'Syne,sans-serif', color: 'var(--text-primary)', position: 'relative', overflow: 'hidden' }}>
      ...
      <nav style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          <div style={{ width: 38, height: 38, borderRadius: 10, fontSize: 20, fontWeight: 800, background: 'linear-gradient(135deg,var(--purple-light),var(--purple))', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', boxShadow: '0 4px 16px rgba(37,99,235,0.2)', flexShrink: 0 }}>₳</div>
          <div>
      ...
      <div style={{ textAlign: 'center', padding: '90px 24px 64px', maxWidth: 760, margin: '0 auto', position: 'relative', zIndex: 1 }}>
        <div className="fade-up" style={{ display: 'inline-block', background: 'var(--bg-hover)', border: '1px solid var(--border)', color: 'var(--purple-dark)', fontSize: 10, padding: '5px 16px', borderRadius: 100, letterSpacing: 1.5, textTransform: 'uppercase', marginBottom: 28 }}>
          ✦ {L.badge}
        </div>
        <h1 className="fade-up delay-1" style={{ fontSize: 58, fontWeight: 700, lineHeight: 1.35, letterSpacing: -2, marginBottom: 22 }}>
          <span className="gradient-text">{L.hero1}<br />{L.hero2}</span>
        </h1>
        <p className="fade-up delay-2" style={{ fontSize: 16, color: 'var(--text-secondary)', lineHeight: 1.75, maxWidth: 560, margin: '0 auto 38px' }}>{L.sub}</p>
        ...
        <div className="fade-up delay-4" style={{ display: 'flex', gap: 16, justifyContent: 'center', marginTop: 48, flexWrap: 'wrap' }}>
          {[['63M', 'Indian SMEs'], ['₹2.4L', 'Annual loss/store'], ['470%', 'Avg ROI'], ['3', 'AI agents']].map(([val, lbl]) => (
            <div key={lbl} style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 12, padding: '12px 20px', backdropFilter: 'blur(12px)', transition: 'all 0.25s ease' }}
              ...
              <div style={{ fontSize: 20, fontWeight: 700, fontFamily: 'DM Mono,monospace', background: 'linear-gradient(135deg,var(--purple-dark),var(--purple-light))', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>{val}</div>
              <div style={{ fontSize: 10, color: 'var(--text-muted)', letterSpacing: 0.5, marginTop: 2 }}>{lbl}</div>
            </div>
          ))}
        </div>
      </div>

      <div className="fade-up delay-5" style={{ maxWidth: 660, margin: '0 auto 72px', padding: '22px 32px', position: 'relative', zIndex: 1, background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 16, backdropFilter: 'blur(12px)', textAlign: 'center' }}>
        <div style={{ fontSize: 24, color: 'var(--purple)', marginBottom: 10 }}>❝</div>
        <div style={{ fontSize: 14, color: 'var(--text-secondary)', fontStyle: 'italic', lineHeight: 1.8 }}>A king should ensure that all trade routes report accurate prices and stocks — so no merchant profits from false information while another suffers from ignorance.</div>
        <div style={{ fontSize: 10, color: 'var(--text-muted)', letterSpacing: 1, marginTop: 12 }}>— Chanakya, Arthashastra, 321 BC</div>
      </div>
      ...
      <div style={{ maxWidth: 960, margin: '0 auto 80px', padding: '0 24px', position: 'relative', zIndex: 1 }}>
        <div className="fade-up" style={{ textAlign: 'center', marginBottom: 44 }}>
          ...
          <p style={{ color: 'var(--text-secondary)', marginTop: 10, fontSize: 14 }}>{L.featSub}</p>
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(260px,1fr))', gap: 16 }}>
          {FEATURES.map((f, i) => (
            <div key={f.title} className={`feature-card fade-up delay-${i + 1}`} onMouseEnter={() => setHovered(i)} onMouseLeave={() => setHovered(null)}>
              ...
              <div style={{ fontSize: 12, color: 'var(--text-secondary)', lineHeight: 1.7 }}>{lang === 'hi' ? f.descHi : lang === 'mr' ? f.descMr : f.desc}</div>
            </div>
          ))}
        </div>
      </div>
      ...
      <div style={{ maxWidth: 760, margin: '0 auto 80px', padding: '0 24px', position: 'relative', zIndex: 1 }}>
        <div style={{ background: 'linear-gradient(135deg, var(--bg-card), var(--bg-hover))', border: '1px solid var(--border)', borderRadius: 20, padding: '44px 40px', textAlign: 'center', position: 'relative', overflow: 'hidden', boxShadow: '0 8px 30px rgba(0,0,0,0.05)' }}>
          <h2 style={{ fontSize: 26, fontWeight: 700, marginBottom: 12 }}>Ready to sync your commerce?</h2>
          <p style={{ color: 'var(--text-secondary)', marginBottom: 28, fontSize: 14 }}>Join 63M+ Indian SMEs eliminating inventory chaos with AI</p>
      ...
      <div style={{ textAlign: 'center', padding: '24px', borderTop: '1px solid var(--border)', fontSize: 11, color: 'var(--text-muted)', position: 'relative', zIndex: 1 }}>
